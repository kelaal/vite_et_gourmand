<?php
/**
 * Action Modification Menu - Administration
 * ECF : Traitement sécurisé formulaire modification menu
 */
...
if (!empty($errors)) {
    ...
    header('Location: ../../admin/modifier_menu.php?id=' . $menuId);
    exit;
}
...
try {
    ...
    if ($updateImage) {
        $stmt = $pdo->prepare("
            UPDATE menu SET
                ...
                nb_personne_min = :nb_personne_min, prix_base_min = :prix_base_min,
                stock_disponible = :stock_disponible, conditions_delai_stockage = :conditions_delai_stockage,
    ...
    // 2. Mise à jour association plats (remplace tout)
    ...
    // 3. Galerie images (ajout seulement, suppression gérée séparément si besoin)
    ...
    $pdo->commit();
    setFlashSuccess('Menu modifié avec succès.');
    header('Location: ../../admin/gestion_menus.php');
    exit;

} catch (Exception $e) {
    $pdo->rollBack();
    setFlashError('Erreur lors de la modification : ' . $e->getMessage());
    header('Location: ../../admin/modifier_menu.php?id=' . $menuId);
    exit;
}
